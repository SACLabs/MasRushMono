import os
import pytest
from masr.ultils.pressor import compress_file, decompress_file  # 确保替换为正确的模块名
...
def test_file_compression():
    ...
    assert os.path.exists(TEST_COMPRESSED_FILE), "Compressed file does not exist."
...
def test_file_decompression():
    ...
    assert os.path.exists(TEST_DECOMPRESSED_FILE), "Decompressed file does not exist."
    with open(TEST_DECOMPRESSED_FILE, 'r') as f:
        content = f.read()
    assert content == 'This is a test file with sample text.', "Decompressed file content does not match."
