...
def test_gen_sender(mocker):
    ...
    mocker.patch('masr.ultils.sender.redis.from_url', return_value=mock_redis_client)

    # Mock requests.post response
    mock_response = mocker.Mock()
    mock_response.raise_for_status.return_value = None
    mock_post = mocker.patch('masr.ultils.sender.requests.post', return_value=mock_response)
    ...
    mock_post.assert_called_once_with("http://example.com:8080", json=mock_message)
